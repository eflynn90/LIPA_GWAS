...
med_tf = read.table("expr/ENSG00000066336.rnaseqc.median.txt",header=TRUE)
...
summary(lm( as.formula( paste0( "value ~ PU.1_SNP_DS + GWAS_SNP_DS +",
                                paste(names(WHLBLD_data)[12:79],collapse='+')) ),
            data=WHLBLD_data))
summary(lm( as.formula( paste0( "value ~ PU.1_SNP_DS * GWAS_SNP_DS +",
                                paste(names(WHLBLD_data)[12:79],collapse='+')) ),
            data=WHLBLD_data))
...
summary(lm( as.formula( paste0( "value ~ PU.1_SNP_DS + GWAS_SNP_DS +",
                                paste(names(SPLEEN_data)[12:46],collapse='+')) ),
            data=SPLEEN_data))
summary(lm( as.formula( paste0( "value ~ PU.1_SNP_DS * GWAS_SNP_DS +",
                                paste(names(SPLEEN_data)[12:46],collapse='+')) ),
            data=SPLEEN_data))
...
summary(lm( as.formula( paste0( "value ~ PU.1_SNP_DS + GWAS_SNP_DS +",
                                paste(names(LCL_data)[12:34],collapse='+')) ),
            data=LCL_data))
summary(lm( as.formula( paste0( "value ~ PU.1_SNP_DS * GWAS_SNP_DS +",
                                paste(names(LCL_data)[12:34],collapse='+')) ),
            data=LCL_data))
...
summary(lm( as.formula( paste0( "value ~ PU.1_SNP_DS + GWAS_SNP_DS +",
                                paste(names(FIBRBLS_data)[12:79],collapse='+')) ),
            data=FIBRBLS_data))
summary(lm( as.formula( paste0( "value ~ PU.1_SNP_DS * GWAS_SNP_DS +",
                                paste(names(FIBRBLS_data)[12:79],collapse='+')) ),
            data=FIBRBLS_data))
...
summary(lm( as.formula( paste0( "value ~ PU.1_SNP_DS + GWAS_SNP_DS +",
                                paste(names(LUNG_data)[12:79],collapse='+')) ),
            data=LUNG_data))
summary(lm( as.formula( paste0( "value ~ PU.1_SNP_DS * GWAS_SNP_DS +",
                                paste(names(LUNG_data)[12:79],collapse='+')) ),
            data=LUNG_data))
...
tiss_lms = do.call('rbind', apply(tiss_trans[-c(7,24,25,31,35),], 1, function(row) {
  ...
  data.frame(tiss=tiss_short, tiss_long=tiss_long, tiss_col=row[4],
             ...
             B_0 = lin_mod$coefficients[1,1],
             med_PU.1_tpm = as.numeric(med_tf[tiss_short]) )
  
}) )

tiss_lms$PU.1_log2_afc <- log2( 2 * tiss_lms$PU.1_B / tiss_lms$B_0 + 1 )
tiss_lms$GWAS_log2_afc <- log2( 2 * tiss_lms$GWAS_B / tiss_lms$B_0 + 1 )
...
tiss_lms %>%
  ggplot(aes(PU.1_log2_afc, GWAS_log2_afc)) +
  geom_point(aes(col=which_sig,fill=which_sig,shape=which_sig)) +
  theme_classic() +
  geom_hline(yintercept=0) +
  geom_vline(xintercept=0) +
...
cor.test(tiss_lms$med_PU.1_tpm, tiss_lms$PU.1_log2_afc, method='spearman')
cor.test(tiss_lms %>% filter(PU.1_B > 0) %>% pull(med_PU.1_tpm), 
         tiss_lms %>% filter(PU.1_B > 0) %>% pull(PU.1_log2_afc), 
         method='spearman')
tiss_lms %>%
  ggplot(aes(log10(med_PU.1_tpm), PU.1_log2_afc)) +
  geom_hline(yintercept = 0) +
  geom_point(aes(col=tiss, fill=tiss, shape=which_sig)) +
  scale_color_manual(values=as.character(tiss_lms$tiss_col)) +
  scale_fill_manual(values=as.character(tiss_lms$tiss_col)) +
  scale_shape_manual(values=c(22,1,25,1)) +
  theme_classic()


cor.test(tiss_lms$med_PU.1_tpm, tiss_lms$GWAS_log2_afc, method='spearman')
cor.test(tiss_lms %>% filter(GWAS_B > 0) %>% pull(med_PU.1_tpm), 
         tiss_lms %>% filter(GWAS_B > 0) %>% pull(GWAS_log2_afc), 
         method='spearman')
tiss_lms %>%
  ggplot(aes(log10(med_PU.1_tpm), GWAS_log2_afc)) +
  geom_hline(yintercept = 0) +
  geom_point(aes(col=tiss, fill=tiss, shape=which_sig)) +
  scale_color_manual(values=as.character(tiss_lms$tiss_col)) +
  scale_fill_manual(values=as.character(tiss_lms$tiss_col)) +
  scale_shape_manual(values=c(22,24,1,1)) +
  theme_classic()


cor.test(tiss_lms$B_0, tiss_lms$PU.1_log2_afc, method='spearman')
cor.test(tiss_lms %>% filter(PU.1_B > 0) %>% pull(B_0), 
         tiss_lms %>% filter(PU.1_B > 0) %>% pull(PU.1_log2_afc), 
         method='spearman')
tiss_lms %>%
  ggplot(aes(log10(B_0), PU.1_log2_afc)) +
  geom_hline(yintercept = 0) +
  geom_point(aes(col=tiss, fill=tiss, shape=which_sig)) +
  scale_color_manual(values=as.character(tiss_lms$tiss_col)) +
  scale_fill_manual(values=as.character(tiss_lms$tiss_col)) +
...
cor.test(tiss_lms$B_0, tiss_lms$GWAS_log2_afc, method='spearman')
cor.test(tiss_lms %>% filter(GWAS_B > 0) %>% pull(B_0), 
         tiss_lms %>% filter(GWAS_B > 0) %>% pull(GWAS_log2_afc), 
         method='spearman')
tiss_lms %>%
  ggplot(aes(log10(B_0), GWAS_log2_afc)) +
  geom_hline(yintercept = 0) +
  geom_point(aes(col=tiss, fill=tiss, shape=which_sig)) +
  scale_color_manual(values=as.character(tiss_lms$tiss_col)) +
  scale_fill_manual(values=as.character(tiss_lms$tiss_col)) +
...
tiss_lms_raw = do.call('rbind', apply(tiss_trans[-c(7,24,25,31,35),], 1, function(row) {
  tiss_short = as.character(row[3])
  tiss_long = as.character(row[2])
  print(tiss_long)
  ...
  lin_mod=summary(lm( as.formula( paste0( "2 ^ value ~ PU.1_SNP_DS + GWAS_SNP_DS +",
                                          paste(names(tiss_data)[which(names(tiss_data) == 'PC1'):which(names(tiss_data)=='sex')],collapse='+')) ),
                      data=tiss_data))
...
tiss_lms_raw$PU.1_log2_afc <- log2( 2 * tiss_lms_raw$PU.1_B / tiss_lms_raw$B_0 + 1 )
tiss_lms_raw$GWAS_log2_afc <- log2( 2 * tiss_lms_raw$GWAS_B / tiss_lms_raw$B_0 + 1 )

tiss_lms_raw %>% filter(PU.1_p < 0.05) %>% pull(tiss_long)
tiss_lms_raw %>% filter(GWAS_p < 0.05) %>% pull(tiss_long)

tiss_lms_raw = tiss_lms_raw %>%
  mutate(sig_info=paste(PU.1_p<0.05,GWAS_p<0.05,sep='_')) %>%
  mutate(either_sig = PU.1_p <0.05 | GWAS_p < 0.05) %>%
  mutate(both_sig = PU.1_p < 0.05 & GWAS_p < 0.05) %>%
  mutate(which_sig = factor(ifelse(PU.1_p < 0.05 & GWAS_p < 0.05,'both',
...
#   ggplot(aes(PU.1_B, GWAS_B)) +
#   geom_point(aes(col=as.character(tiss), shape=either_sig)) +
#   scale_color_manual(values=as.character(tiss_lms_raw$tiss_col)) +
#   theme_classic() +
#   geom_hline(yintercept=0) +
#   geom_vline(xintercept=0)
# 
...
tiss_lms_raw %>%
  ...
  geom_point(aes(col=tiss, fill=tiss, shape=which_sig)) +
  scale_color_manual(values=as.character(tiss_lms_raw$tiss_col)) +
  scale_fill_manual(values=as.character(tiss_lms_raw$tiss_col)) +
  scale_shape_manual(values=c(22,24,25,1)) +
  theme_classic() +
  geom_hline(yintercept=0) +
  geom_vline(xintercept=0)
...
cor.test(tiss_lms_raw$med_PU.1_tpm, tiss_lms_raw$PU.1_log2_afc, method='spearman')
cor.test(tiss_lms_raw %>% filter(PU.1_B > 0) %>% pull(med_PU.1_tpm), 
         tiss_lms_raw %>% filter(PU.1_B > 0) %>% pull(PU.1_log2_afc), 
         method='spearman')
tiss_lms_raw %>%
  ggplot(aes(log10(med_PU.1_tpm), PU.1_log2_afc)) +
  geom_hline(yintercept = 0) +
...
cor.test(tiss_lms_raw$med_PU.1_tpm, tiss_lms_raw$GWAS_log2_afc, method='spearman')
cor.test(tiss_lms_raw %>% filter(GWAS_B > 0) %>% pull(med_PU.1_tpm), 
         tiss_lms_raw %>% filter(GWAS_B > 0) %>% pull(GWAS_log2_afc), 
         method='spearman')
tiss_lms_raw %>%
  ggplot(aes(log10(med_PU.1_tpm), GWAS_log2_afc)) +
  geom_hline(yintercept = 0) +
...
tiss_lms_raw_nocov = do.call('rbind', apply(tiss_trans[-c(7,24,25,31,35),], 1, function(row) {
  tiss_short = as.character(row[3])
  tiss_long = as.character(row[2])
  print(tiss_long)
  ...
  lin_mod=summary(lm( 2 ^ value ~ PU.1_SNP_DS + GWAS_SNP_DS,
                      data=tiss_data))
...
tiss_lms_raw_nocov$PU.1_log2_afc <- log2( 2 * tiss_lms_raw_nocov$PU.1_B / tiss_lms_raw_nocov$B_0 + 1 )
tiss_lms_raw_nocov$GWAS_log2_afc <- log2( 2 * tiss_lms_raw_nocov$GWAS_B / tiss_lms_raw_nocov$B_0 + 1 )

tiss_lms_raw_nocov %>% filter(PU.1_p < 0.05) %>% pull(tiss_long)
tiss_lms_raw_nocov %>% filter(GWAS_p < 0.05) %>% pull(tiss_long)

tiss_lms_raw_nocov = tiss_lms_raw_nocov %>%
  mutate(sig_info=paste(PU.1_p<0.05,GWAS_p<0.05,sep='_')) %>%
  mutate(either_sig = PU.1_p <0.05 | GWAS_p < 0.05) %>%
  mutate(both_sig = PU.1_p < 0.05 & GWAS_p < 0.05) %>%
  mutate(which_sig = factor(ifelse(PU.1_p < 0.05 & GWAS_p < 0.05,'both',
...
#   ggplot(aes(PU.1_B, GWAS_B)) +
#   geom_point(aes(col=as.character(tiss), shape=either_sig)) +
#   scale_color_manual(values=as.character(tiss_lms_raw_nocov$tiss_col)) +
#   theme_classic() +
#   geom_hline(yintercept=0) +
#   geom_vline(xintercept=0)
# 
...
tiss_lms_raw_nocov %>%
  ...
  geom_point(aes(col=tiss, fill=tiss, shape=which_sig)) +
  scale_color_manual(values=as.character(tiss_lms_raw_nocov$tiss_col)) +
  scale_fill_manual(values=as.character(tiss_lms_raw_nocov$tiss_col)) +
  scale_shape_manual(values=c(22,24,25,1)) +
  theme_classic() +
  geom_hline(yintercept=0) +
  geom_vline(xintercept=0)
...
cor.test(tiss_lms_raw_nocov$med_PU.1_tpm, tiss_lms_raw_nocov$PU.1_log2_afc, method='spearman')
cor.test(tiss_lms_raw_nocov %>% filter(PU.1_B > 0) %>% pull(med_PU.1_tpm), 
         tiss_lms_raw_nocov %>% filter(PU.1_B > 0) %>% pull(PU.1_log2_afc), 
         method='spearman')
tiss_lms_raw_nocov %>%
  ggplot(aes(log10(med_PU.1_tpm), PU.1_log2_afc)) +
  geom_hline(yintercept = 0) +
...
cor.test(tiss_lms_raw_nocov$med_PU.1_tpm, tiss_lms_raw_nocov$GWAS_log2_afc, method='spearman')
cor.test(tiss_lms_raw_nocov %>% filter(GWAS_B > 0) %>% pull(med_PU.1_tpm), 
         tiss_lms_raw_nocov %>% filter(GWAS_B > 0) %>% pull(GWAS_log2_afc), 
         method='spearman')
tiss_lms_raw_nocov %>%
  ggplot(aes(log10(med_PU.1_tpm), GWAS_log2_afc)) +
  geom_hline(yintercept = 0) +
